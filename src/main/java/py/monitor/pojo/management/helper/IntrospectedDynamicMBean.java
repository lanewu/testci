 ...
 * @author sxl
 * 
 * @history 1.sxl Add notification mechanism to the dynamic mbean </br>
 * 
 */
public class IntrospectedDynamicMBean extends NotificableDynamicMBean {
    ...
    public void setAttribute(Attribute attribute)
        ...
        try {
            ...
            // AttributeChangeNotification.ATTRIBUTE_CHANGE, "Server Host Change", this.getClass().getName(), null,
            // null);
            ...
            throw new ReflectionException(e,
                    format("attribute %s of %s, value = (%s)%s", name, mbeanClass, value.getClass().getName(), value));
        } catch (InvocationTargetException e) {
            throw new MBeanException((Exception) e.getCause(),
                    format("attribute %s of %s, value = (%s)%s", name, mbeanClass, value.getClass().getName(), value));
        }
    ...
    private static Map<String, Method> createOperationMethods(BeanInfo beanInfo) throws ManagementException {
        ...
        for (MethodDescriptor descriptor : beanInfo.getMethodDescriptors()) {
            ...
            /*
             * consider the Method an automatic operation if it satisfies all of: 1. its class is annotated
             * with @MBean(automatic=OPERATION) 2. it is a public instance (non static) method 3. it is NOT considered a
             * bean accessor (getter/setter)
             */
     ...
     * 
     * @param propertyDescriptors
     *            property descriptors that are known to have at least one {@link ManagedAttribute} annotation on its
    ...
    private static MBeanAttributeInfo[] createAttributeInfo(Map<String, PropertyDescriptor> propertyDescriptors)
        ...
        for (String propertyName : sortedKeys(propertyDescriptors)) {
            ...
            if (getterAnnotation != null) {
                // format all the annotation informations to description
    ...
    public String toString() {
        return "IntrospectedDynamicMBean [pojo=" + pojo + ", mbeanClass=" + mbeanClass + ", registrationDelegate="
                + registrationDelegate + ", propertyDescriptors=" + propertyDescriptors + ", operationMethods="
