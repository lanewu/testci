...
/***
 * A factory for creating a pair of protocols (one for input, one for output). Gives more flexibility to the factory to
 * enforce protocol restrictions (e.g. header protocol requires the input/output transport/protocol are the same, a
 * duplex header protocol factory can enforce the transport restriction because it can see both transports, and can
 * guarantee both protocols are the same because it can fill in the input/output slots of the protocol pair with the
...
public abstract class TDuplexProtocolFactory {
    ...
    public static TDuplexProtocolFactory fromSeparateFactories(final TProtocolFactory inputProtocolFactory,
