...
public class ShutdownUtil {
    ...
    public static void shutdownChannelFactory(ChannelFactory channelFactory, ExecutorService bossExecutor,
    ...
    public static void closeChannels(ChannelGroup allChannels) {
        if (allChannels.size() > 0) {
            // TODO : allow an option here to control if we need to drain connections and wait instead of killing them
            // all
            try {
    ...
    public static void shutdownExecutor(ExecutorService executor, final String name) {
        ...
        try {
            ...
            if (!executor.awaitTermination(500, TimeUnit.SECONDS)) {
                log.warn(name + " did not shutdown properly, shutdown: " + executor.isShutdown() + ", terminated: "
