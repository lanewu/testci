...
public class NiftyProcessorAdapters {
    private final static Logger logger = LoggerFactory.getLogger(NiftyProcessorAdapters.class);
    private static Field ifaceFiled;
    
    private static String metricPrefix = "Server_process";
    private static PYMetricRegistry metricRegistry = PYMetricRegistry.getMetricRegistry();
    private static Map<String, PYMetric> metricsMap = new ConcurrentHashMap<String, PYMetric>();
    private static PYMetric processMeter = metricRegistry.register(String.format("[%s]%s", metricPrefix, "processMeter"),
            com.codahale.metrics.Meter.class);

    public static PYMetric generateNewMetrics(String className, String methodName) {
        PYMetric newMetric = metricRegistry.register(String.format("[%s]%s_%s", metricPrefix, className, methodName),
                        com.codahale.metrics.Timer.class);
        return newMetric;
    }

    private static PYTimerContext registerTimer(PYMetric metric) {
        PYTimerContext context = null;
        if (metric != null) {
            context = metric.time();
        }
        return context;
    ...
    private static void stopTimer(PYTimerContext context) {
        if (context != null) {
            context.stop();
        }
    }
    ...
    /**
     * Adapt a {@link TProcessor} to a standard Thrift {@link NiftyProcessor}. Nifty uses this internally to adapt the
     * processors generated by the standard Thrift code generator into instances of {@link NiftyProcessor} usable by
     * {@link com.facebook.nifty.core.NiftyDispatcher}
     */
    public static NiftyProcessor processorFromTProcessor(final TProcessor standardThriftProcessor) {
        ...
        return new NiftyProcessor() {
            ...
            public ListenableFuture<Boolean> process(TProtocol in, TProtocol out, RequestContext requestContext)
                            throws TException {
                /**
                 * for catching all exceptions include which are not derived from TException, we implements our the
                 * function of process().
                 */
                if (ifaceFiled == null) {
                    try {
                ...
                try {
                    if (fn == null) {
                        ...
                        PYMetric metric = null;
                        if (metricsMap.containsKey(methodName)) {
                            metric = metricsMap.get(methodName);
                        } else {
                            String className = standardThriftProcessor.getClass().getSimpleName();
                            metric = generateNewMetrics(className, methodName);
                            metricsMap.put(methodName, metric);
                        }
                        PYTimerContext pyContext = null;
                        try {
                            pyContext = registerTimer(metric);
    ...
    /**
     * Create a {@link NiftyProcessorFactory} that always returns the same {@link NiftyProcessor} adapted from the given
    ...
    /**
     * Create a {@link NiftyProcessorFactory} that delegates to a standard Thrift {@link TProcessorFactory} to construct
     * an instance, then adapts each instance to a {@link NiftyProcessor}
     */
    public static NiftyProcessorFactory factoryFromTProcessorFactory(
    ...
    /**
     * Adapt a {@link NiftyProcessor} to a standard Thrift {@link TProcessor}. The
     * {@link com.facebook.nifty.core.NiftyRequestContext} will always be {@code null}
     */
    ...
    /**
     * Create a standard thrift {@link TProcessorFactory} that always returns the same {@link TProcessor} adapted from
     * the given {@link NiftyProcessor}
     */
    ...
    /**
     * Create a standard thrift {@link TProcessorFactory} that delegates to a {@link NiftyProcessorFactory} to construct
     * an instance, then adapts each instance to a standard Thrift {@link TProcessor}
     */
    ...
    /**
     * Catch the mismatch early if someone tries to pass our internal variant of TProcessor with a different signature
     * on the process() method into these adapters.
     */
