...
/**
 * list_swap - swap two node in list
 * @tmp1 : the element
 * @tmp2 : the element
 */
				...
				struct list_head *tmp2)
{
    ...
    //get tmp1 next and prev
    tmp1_next = tmp1->next ;
    tmp1_prev = tmp1->prev ;
    //get tmp2 next and prev
    tmp2_next = tmp2->next ;
    tmp2_prev = tmp2->prev ;
    
    if(tmp1 == tmp2) // same node
    {
        return 0 ;   
    }    
    else if(tmp1->next == tmp2 && tmp1->prev == tmp2) //only two node list
    {
        return 0 ; // not need swap, not execute in this list
    }    
    else if(tmp1->next == tmp2 && tmp1->prev != tmp2) 
    {
         //A->tmp1->tmp2->D
         //after swap A->tmp2->tmp1->D
        tmp1_prev->next = tmp2 ;
        tmp2->prev = tmp1_prev ;
        tmp2->next = tmp1 ;
        tmp1->prev = tmp2 ;
